...
// Keep the per-request context aligned with the streaming HTTP client timeout.
// Large Claude Code contexts can take well over 90s before the upstream stream
// finishes, and a shorter context timeout surfaces as "context deadline exceeded"
// in Claude's retry UI even when the model/backend is still working.
const kiroRequestTimeout = 5 * time.Minute
...
func CallKiroAPI(account *config.Account, payload *KiroPayload, callback *KiroStreamCallback) error {
	...
	logger.Infof("[KiroAPI] trace=%s start model=%q account=%q history=%d contentLen=%d", traceID, modelID, accountEmail, historyLen, contentLen)
	defer func() {
		logger.Infof("[KiroAPI] trace=%s done total=%s", traceID, time.Since(startedAt).Round(time.Millisecond))
	}()
	...
	for _, ep := range endpoints {
		...
		reqCtx, cancelReq := context.WithTimeout(context.Background(), kiroRequestTimeout)
		...
		logger.Infof("[KiroAPI] trace=%s endpoint=%s send bytes=%d", traceID, ep.Name, len(reqBody))
		...
		logger.Infof("[KiroAPI] trace=%s endpoint=%s status=%d openAfter=%s", traceID, ep.Name, resp.StatusCode, time.Since(endpointStartedAt).Round(time.Millisecond))
