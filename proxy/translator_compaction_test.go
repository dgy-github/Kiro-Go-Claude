...
func TestClaudeToKiroFlattensActiveToolResultWhenToolsOmitted(t *testing.T) {
	req := &ClaudeRequest{
		Model: "claude-opus-4.8",
		Messages: []ClaudeMessage{
			{Role: "user", Content: "run ls"},
			{Role: "assistant", Content: []interface{}{
				map[string]interface{}{"type": "tool_use", "id": "t10", "name": "exec_command", "input": map[string]interface{}{"cmd": "ls"}},
			}},
			{Role: "user", Content: []interface{}{
				map[string]interface{}{"type": "tool_result", "tool_use_id": "t10", "content": "file1 file2"},
			}},
		},
	}
	...
	for _, h := range hist {
		if h.AssistantResponseMessage != nil && len(h.AssistantResponseMessage.ToolUses) > 0 {
			t.Fatalf("expected active tool use to be flattened when no tools are supplied, got %d tool uses", len(h.AssistantResponseMessage.ToolUses))
		}
	}

	cur := payload.ConversationState.CurrentMessage.UserInputMessage
	if cur.UserInputMessageContext != nil && len(cur.UserInputMessageContext.ToolResults) > 0 {
		t.Fatalf("expected current tool results to be flattened when no tools are supplied")
	}
	if !strings.Contains(cur.Content, "file1 file2") {
		t.Fatalf("expected flattened current content to contain tool output, got %q", cur.Content)
	}
}
