...
const maxPayloadBytes = 700 * 1024
...
func ClaudeToKiro(req *ClaudeRequest, thinking bool) *KiroPayload {
	...
	keepCurrentToolResults := currentToolResultsMatchLastAssistant(history, currentToolResultIDs) &&
		(len(req.Tools) > 0 || len(currentImages) > 0)
	...
	// Image-bearing current tool results are kept too, because the text
	// placeholder alone cannot carry the image payload.
	var attachToolResults []KiroToolResult
	if keepCurrentToolResults || (len(currentToolResults) > 0 && len(currentImages) > 0) {
...
func applyPromptFilters(prompt string) string {
	...
	projectContext := configuredProjectContextLine()
	if projectContext == "" {
		projectContext = extractProjectContextLine(prompt)
	}
	...
	if projectContext != "" {
		prompt = appendProjectContextLine(prompt, projectContext)
	}
...
func stripEnvNoiseLines(prompt string) string {
	...
	out := make([]string, 0, len(lines))
	skipSection := false
	configuredProjectContext := configuredProjectContextLine()
	for _, line := range lines {
		trimmed := strings.TrimSpace(line)
		lower := strings.ToLower(trimmed)
		projectContext := extractProjectContextLine(trimmed)
		...
		if skipSection {
			...
			} else if projectContext != "" && configuredProjectContext == "" {
...
// extractProjectContextLine keeps the one environment detail Claude Code needs
// to avoid drifting into sibling projects after prompt noise filtering.
func extractProjectContextLine(prompt string) string {
	patterns := []*regexp.Regexp{
		regexp.MustCompile(`(?im)^\s*(?:cwd|current\s+working\s+directory|working\s+directory|project\s+root)\s*[:=]\s*` + "`?" + `([A-Za-z]:\\[^\r\n` + "`" + `]+)` + "`?" + `\s*$`),
		regexp.MustCompile(`(?im)^\s*(?:cwd|current\s+working\s+directory|working\s+directory|project\s+root)\s+is\s+` + "`?" + `([A-Za-z]:\\[^\r\n` + "`" + `]+)` + "`?" + `\s*$`),
	}
	for _, re := range patterns {
		if m := re.FindStringSubmatch(prompt); len(m) == 2 {
			return "Current project root: " + strings.TrimSpace(m[1])
		}
	}
	return ""
}
...
func appendProjectContextLine(prompt, projectContext string) string {
	...
	for _, line := range lines {
		if extractProjectContextLine(line) != "" {
			continue
		}
		out = append(out, line)
	}
...
func shouldForceToolUseAfterContinuation(currentUser, previousAssistant string) bool {
	...
	prev := strings.ToLower(previousAssistant)
	if prev == "" {
		return false
	}
...
Never write fake tool transcripts such as "Tool results:", "[Read]", "[Bash]", or "[Edit]" in assistant text. When more file inspection or shell work is needed, emit real structured tool_use calls; otherwise provide the final answer.
When a Current project root is provided, treat it as the active repository root and read HANDOFF.md from that directory first. Do not infer a sibling or parent project from CLAUDE.md, AGENTS.md, search results, or old handoff files.
Default to implementing the smallest useful change. Do not stop to ask scope questions when the user already gave a concrete next action; proceed, state assumptions briefly, and verify.
Readonly diagnosis is pre-authorized: reading files, reading logs, listing directories, grepping code, and inspecting local session JSONL do not require confirmation. Do not ask "continue?", "confirm?", or "may I read?" for readonly investigation; issue the real tool_use calls directly.
When the user explicitly authorizes an action with phrases like "可以删除", "继续", "直接做", "精简", or "按你的判断", do not ask a second confirmation about what to delete or what to keep. Apply the smallest safe version of that authorization and report what changed.`
...
func convertClaudeTools(tools []ClaudeTool) ([]KiroToolWrapper, map[string]string) {
	...
	for _, tool := range tools {
		if shouldDropClaudeTool(tool.Name) {
			continue
		}
...
func shouldDropClaudeTool(name string) bool {
...
// isPollutedAssistantToolResultsText detects assistant turns where the model
// copied Claude Code's rendered tool-result transcript (for example
// "Tool results:\n\n[Read] ...") instead of issuing structured tool_use blocks.
// Replaying those turns teaches the model to keep faking tool results, so they
// are treated as hollow assistant turns and dropped from upstream history.
func isPollutedAssistantToolResultsText(content string) bool {
	trimmed := strings.TrimSpace(content)
	if !strings.HasPrefix(trimmed, toolResultsContinuationPrefix) {
		return false
	}
	return pollutedAssistantToolResultsPattern.MatchString(trimmed)
}
	...
	for i := range history {
		...
		if msg.AssistantResponseMessage != nil {
			...
			if msg.AssistantResponseMessage.Content != "" {
				if isPollutedAssistantToolResultsText(msg.AssistantResponseMessage.Content) {
...
func truncatePayloadToLimit(payload *KiroPayload, hasPriming bool) {
	...
	// The summary is intentionally more informative than the old one-line
	// placeholder. If that extra context pushes us over the byte limit, trim the
	// oldest retained tail entries after the summary until the request fits.
	for payloadByteSize(payload) > maxPayloadBytes {
		summaryIdx := primingCount
		tailStart := summaryIdx + 1
		if keepFrom == 0 || len(payload.ConversationState.History) <= tailStart+minRecentHistoryTurns {
			break
		}
	...
	logger.Infof(
		"[KiroPayload] truncated bytes=%d->%d history=%d->%d limit=%d",
		beforeBytes,
		afterBytes,
		beforeHistory,
		afterHistory,
...
func buildCompactionSummary(dropped []KiroHistoryMessage) string {
	...
	selected := selectCompactionSnippets(snippets)
	var b strings.Builder
	fmt.Fprintf(&b, "[Earlier conversation was compacted by Kiro-Go before forwarding upstream. Dropped history entries: %d; user turns: %d; assistant turns: %d.]\n", len(dropped), userTurns, assistantTurns)
	if len(selected) > 0 {
		b.WriteString("Important retained snippets from the compacted portion:\n")
		for _, s := range selected {
			b.WriteString("- ")
			b.WriteString(s)
			b.WriteString("\n")
			if b.Len() >= compactionSummaryMaxChars {
				break
			}
		}
	}
	...
	if len(out) > compactionSummaryMaxChars {
		out = out[:compactionSummaryMaxChars] + "\n[Compaction summary truncated.]"
	}
	return out
}

func compactHistorySnippet(role, content string) string {
	content = strings.TrimSpace(collapseWhitespace(content))
	if content == "" || content == minimalFallbackUserContent {
		return ""
	}
	if len(content) > compactionSnippetMaxChars {
		content = content[:compactionSnippetMaxChars] + "..."
	}
	return role + ": " + content
}

func selectCompactionSnippets(snippets []string) []string {
	if len(snippets) <= 18 {
		return snippets
	}
	out := make([]string, 0, 18)
	out = append(out, snippets[:6]...)
	out = append(out, "... compacted middle omitted ...")
	out = append(out, snippets[len(snippets)-11:]...)
	return out
}
